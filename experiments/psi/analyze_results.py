...
def load_data() -> pd.DataFrame:
    df = pd.read_csv(
        CSV_PATH,
        header=None,
        names=["commit", "job_index", "x_size", "y_size", "i_size", "mean_ms", "stddev_ms"],
    )
...
def save_plot_heatmap(df: pd.DataFrame) -> None:
    aggregated = (
        df.groupby(["y_size", "x_size"], as_index=False)["mean_ms"].mean().rename(columns={"mean_ms": "mean_over_i_ms"})
    )
    pivot = aggregated.pivot(index="y_size", columns="x_size", values="mean_over_i_ms")
...
def write_summary(df: pd.DataFrame) -> None:
    unbalanced = df[(df["y_size"] == 1) & (df["i_size"] == 1)].sort_values("x_size")
    fixed_y = df[(df["x_size"] == 2048) & (df["i_size"] == 1)].sort_values("y_size")
    ...
    lines = [
        ...
        (
            f"- Unbalanced case (|Y|=1, |I|=1): runtime rises from {unbalanced['mean_ms'].iloc[0]} ms at |X|={unbalanced['x_size'].iloc[0]} "
            f"to {unbalanced['mean_ms'].iloc[-1]} ms at |X|={unbalanced['x_size'].iloc[-1]}."
        ),
        (
            f"- Fixed |X|=2048 with |I|=1: runtime rises from {fixed_y['mean_ms'].iloc[0]} ms at |Y|={fixed_y['y_size'].iloc[0]} "
            f"to {fixed_y['mean_ms'].iloc[-1]} ms at |Y|={fixed_y['y_size'].iloc[-1]}."
        ),
